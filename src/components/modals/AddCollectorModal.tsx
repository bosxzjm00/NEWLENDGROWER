...
export const AddCollectorModal: React.FC<AddCollectorModalProps> = ({ isOpen, onClose }) => {
  const { addCollector } = useApp();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addCollector(name.trim(), contact.trim(), address.trim());
    onClose();
    setName('');
    setContact('');
    setAddress('');
      ...
      <div className="card-bg w-full max-w-md p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xl">
        <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-200 dark:border-slate-800">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-slate-900 dark:text-white font-bold text-sm">New Collector Profile Registration</h3>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1 font-medium">Collector Full Name *</label>
            <input
              type="text"
              required
              value={name}
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g., Brgy. Maligaya, Pasig City"
              className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 font-medium"
            />
          </div>
